...
module Verdandi
  ...
  class BoundariesScraper
    ...
    def self.clear
      REDIS.del %w{aqa edexcel ocr}
    end

    def self.aqa
      urls = %w{http://web.aqa.org.uk/exams-office/about-results/grade-boundaries.php http://web.aqa.org.uk/exams-office/about-results/grade-boundaries-archive.php}

      boundaries_links = urls.map do |url|
        data = Nokogiri::HTML(open(url))
        data.xpath "//div[starts-with(@id, 'mat') and (string-length(@id) = 4 or string-length(@id) = 5)]/ul[@class='links']/li/a"
      end

      boundaries_links.flatten!
      boundaries_links.map! { |link| link[:href] }
      boundaries_links.select! { |link| link =~ /.*AQA-(APPLIED-)?(APPL-)?(GCSE|A-LEVEL).*/ }

      boundaries_links.each { |link| save_url link, :aqa }
    end

    def self.edexcel
      urls = %w{http://www.edexcel.com/iwantto/pages/gce-grade-boundaries.aspx http://www.edexcel.com/iwantto/pages/gcse-grade-boundaries.aspx}

      boundaries_links = urls.map do |url|
        data = Nokogiri::HTML(open(url))
        data.xpath "//tr[@id='tableSetup']/td[@id='edx-FullLayoutLeft']/table/tr/td/table/tr[2]/td/div/table/tr/td/a"
      end

      boundaries_links.flatten!
      boundaries_links.map! { |link| URI.encode "http://www.edexcel.com#{link[:href]}" }

      boundaries_links.each { |link| save_url link, :edexcel }
    end

    def self.ocr
      url = "http://www.ocr.org.uk/administration/results/results_stats/"
      data = Nokogiri::HTML(open(url))
      boundaries_links = data.xpath "//li[@class='pdfLink']/a"
      # No map! on Nokogiri::XML::NodeSet
      boundaries_links = boundaries_links.map { |link| "http://www.ocr.org.uk#{link[:href]}" }

      boundaries_links.each { |link| save_url link, :ocr }
    end

    protected
    def self.save_url(url, awarding_body)
      REDIS.lpush "#{awarding_body}_boundary_pdfs", open(url).read
    end
  end
end
